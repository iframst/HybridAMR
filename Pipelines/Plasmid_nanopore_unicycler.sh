...
## Bioinformatics pipleline for nanopore only readss AMR Sequencing Project (UGA - Dr. Grazieli Maboni)
...
## Assemble
unicycler -1 trimmed/${runID}_trimmed.fastq -o assembled_raw

## Map
minimap2 -x map-ont --secondary=no -t 8 assembled_raw/assembly.fasta trimmed/${runID}_trimmed.fastq > temp/Polish_ONT.paf

## Polish
racon trimmed/${runID}_trimmed.fastq temp/Polish_ONT.paf assembled_raw/assembly.fasta -t 8 > ${runID}_out/${runID}_polished.fasta
